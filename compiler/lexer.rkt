...
(define-tokens datum-tokens ( DATUM NONE Identifier Num Keyword KeywordSequence STString STChar Integer Double OperatorSequence))
...
(define som-lexer
  (lexer-src-pos
   ...
   [char (token-STChar lexeme)]
   [(:: identifier ":") (token-Keyword (string->symbol lexeme))]
   ;[keword (token-Keyword (string->symbol lexeme))]
   ;[keyword-sequence (token-KeywordSequence (string->symbol lexeme))]
   [(:+ (:: identifier ":")) (token-KeywordSequence (string->symbol lexeme))]
   [(:: "primitive") 'Primitive]
   ...
   [separator 'Separator]
   [double (token-Double (string->number lexeme))]
...
(define-lex-abbrevs
  ...
  [separator (:>= 3 "-")];don't know the exact number
  ...
  [double (:: (:+ decimal-digit) #\. (:+ decimal-digit))]
  [decimal-digit (:/ #\0 #\9)]
